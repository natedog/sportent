class Vote < ActiveRecord::Base
  ...
  named_scope :popular_stories,:select => 'DISTINCT ON (story_id) story_id,created_at,id',  :order=> "created_at DESC,story_id", :limit=>10
  named_scope :popular_tags, :select => 'DISTINCT ON (social_tag_id) social_tag_id,created_at,id',  :order=> "created_at DESC,social_tag_id" ,:limit=>10
